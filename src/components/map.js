...
useEffect(() => {
    ...
    function populatedMap(userLngLat) {
        ...
        for (let detail of details) {
            let marker = new maplibregl.Marker({ draggable: false }).setLngLat([detail.point[0],
                detail.point[1]]).setPopup(new maplibregl.Popup().setHTML(`<p>Time: ${detail.time}</p><p>Contact:
                <a href="${detail.contact}" target="_blank">${detail.contact}</a></p>`)).addTo(map.current)
        ...
        map.current.fitBounds([[Math.min(...lngs), Math.min(...lats)], [Math.max(...lngs),
            Math.max(...lats)]], { padding: 30 })
    }


    existingUser ? populatedMap([currentUserDetails.point[0], currentUserDetails.point[1]]) : 
        emptyMap()
})
    return (
